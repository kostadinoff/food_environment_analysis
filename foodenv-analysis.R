...
theme_nice <- ggthemes::theme_tufte() +
  theme(
    ...
    plot.subtitle = element_text(family = "Roboto Condensed", size = 22),
    ...
    axis.text = element_text(family = "Roboto Condensed", size = 20),
    axis.text.x = element_text(margin = margin(5, b = 10)),
    strip.text = element_text(family = "Roboto Condensed", size = 20), 
...
######---Data---######
d <- read_excel("SRIPD_Analysis_dataset_recoded_FoodEnv_15.11.24.xls") |> 
  janitor::clean_names() |> 
  mutate(children_household = as.factor(children_household))

# Examine the variables in the dataset (univariate analysis)

if (!dir.exists("figures")) {
  dir.create("figures")
}

# For loop to generate and save plots for each column in the dataset
for (variable in colnames(d)) {
  # Check if the column is numeric, categorical (factor), or character
  if (is.numeric(d[[variable]]) || is.factor(d[[variable]]) || is.character(d[[variable]])) {
    # Generate the plot
    plot <- univariate_plot(d, {{ variable }})
    
    # Save the plot with a custom name
    ggsave(filename = paste0("figures/univariate_plot_", variable, ".png"), 
           plot = plot, 
           bg = "white",
           dpi = 300, 
           width = 16, 
    ...
    message(paste("Skipping non-numeric/categorical column:", variable))
  }
}

d |> 
  select(
  foodenv_distance_alcoholshop,
  ffq_alcohol,
) |> 
  mutate(ffq_alcohol_numeric = case_when(
    ffq_alcohol == "Several times a week (5)" ~ 5,
    ffq_alcohol == "Once a week (4)" ~ 4,
    ffq_alcohol == "Once a month (2)" ~ 2,
    ffq_alcohol == "Never (0)" ~ 0,
    ffq_alcohol == "Every day (6)" ~ 6,
    TRUE ~ NA_real_  # Handle any missing or unexpected values
  )) |>
  lm(ffq_alcohol_numeric ~ foodenv_distance_alcoholshop, data = _) |> 
  tidy() 
